...
function rejectInlineSecrets(value: unknown, path = 'settings'): void {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    if (FORBIDDEN_SECRET_KEY.test(key)) throw new AgentServiceError(400, 'INLINE_SECRET_REJECTED', `${path}.${key} must be a customer-vault reference, not secret material`)
    rejectInlineSecrets(item, `${path}.${key}`)
  }
}

function requireRole(principal: PortalPrincipal, role: EntraAppRole): void {
  if (!hasAppRole(principal, role)) throw new AgentServiceError(403, 'ENTRA_ROLE_REQUIRED', `${role} is required`)
}

function cleanText(value: unknown, name: string, maximum = 256): string {
  if (typeof value !== 'string' || !value.trim() || value.length > maximum) throw new AgentServiceError(400, 'INVALID_INPUT', `${name} is required and must not exceed ${maximum} characters`)
  return value.trim()
}

function optionalEndpoint(value: unknown, mode: AgentConfig['mode']): string | undefined {
...
function optionalCredentialReference(value: unknown): string | undefined {
  ...
  try { url = new URL(reference) } catch {
    throw new AgentServiceError(400, 'INVALID_CREDENTIAL_REFERENCE', 'credentialRef must be an opaque customer-vault, certificate, or managed-identity URI')
  }
...
function record(value: unknown, name: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new AgentServiceError(400, 'INVALID_INPUT', `${name} must be an object`)
...
function timestamp(value: unknown, name: string): string {
  ...
  if (Number.isNaN(date.getTime())) throw new AgentServiceError(400, 'INVALID_INPUT', `${name} must be an ISO-8601 timestamp`)
  return date.toISOString()
}

function confidence(value: unknown, path: string): number | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) throw new AgentServiceError(400, 'INVALID_INPUT', `${path} must be between zero and one`)
  return value
}
...
export class AgentService {
  ...
  createIntegration(principal: PortalPrincipal, catalogId: unknown, input: Record<string, unknown>): IntegrationConfiguration {
    ...
    const create: CreateIntegrationInput = {
      ...
      settings: Object.fromEntries(Object.entries(settings as Record<string, unknown>).map(([key, value]) => {
        if (!['string', 'number', 'boolean'].includes(typeof value)) throw new AgentServiceError(400, 'INVALID_INPUT', `settings.${key} must be a string, number, or boolean`)
  ...
  async testIntegration(principal: PortalPrincipal, id: string): Promise<IntegrationConfiguration> {
    ...
    if (!entry) throw new AgentServiceError(409, 'CONNECTOR_REMOVED', 'Connector manifest no longer exists')
    ...
    if (requirements.length) throw new AgentServiceError(409, 'CONFIGURATION_INCOMPLETE', `Configuration test requires ${requirements.join(' and ')}`)
    ...
    if (runtimeResult && (!runtimeResult.reachable || !runtimeResult.authenticated)) {
      ...
      throw new AgentServiceError(409, 'CONNECTOR_TEST_FAILED', runtimeResult.message)
    }
  ...
  activateIntegration(principal: PortalPrincipal, id: string): IntegrationConfiguration {
    ...
    if (entry && ['pull', 'hybrid'].includes(entry.syncMode) && !this.syncRuntime?.supports(integration)) {
      throw new AgentServiceError(409, 'CONNECTOR_DRIVER_UNAVAILABLE', 'A connector driver must be installed before this pull integration can be activated')
    }
  ...
  requestSync(principal: PortalPrincipal, id: string): SyncJob {
    requireRole(principal, 'Papyrus.Integration.Manage')
    const integration = this.integration(id)
    if (integration.state !== 'active') throw new AgentServiceError(409, 'INTEGRATION_NOT_ACTIVE', 'Only active integrations can be synchronized')
    const entry = catalogEntry(integration.catalogId)
    if (!entry || !['pull', 'hybrid'].includes(entry.syncMode)) throw new AgentServiceError(409, 'INTEGRATION_NOT_PULL_BASED', 'This integration receives pushed observations and does not run scheduled synchronization')
    const job = this.syncRuntime?.enqueue(integration)
    if (!job) throw new AgentServiceError(409, 'CONNECTOR_DRIVER_UNAVAILABLE', 'A connector driver is not installed for this integration')
    return job
  }
  ...
  authorizeIngestionToken(principal: PortalPrincipal, id: string): void {
    requireRole(principal, 'Papyrus.Integration.Manage')
    const integration = this.integration(id)
    if (integration.state !== 'active') throw new AgentServiceError(409, 'INTEGRATION_NOT_ACTIVE', 'Ingestion credentials are issued only for active integrations')
    const entry = catalogEntry(integration.catalogId)
    if (!entry || (!entry.observationProtocol && !['push', 'hybrid'].includes(entry.syncMode))) {
      throw new AgentServiceError(409, 'INTEGRATION_NOT_SIGNAL_SOURCE', 'This plugin does not accept pushed observations or signals')
    }
  }
  ...
  private ingestAuthorizedObservation(id: string, value: Record<string, unknown>) {
    const integration = this.integration(id)
    if (integration.state !== 'active') throw new AgentServiceError(409, 'INTEGRATION_NOT_ACTIVE', 'Observations are accepted only from active integrations')
    const entry = catalogEntry(integration.catalogId)
    if (!entry?.observationProtocol) throw new AgentServiceError(409, 'INTEGRATION_NOT_OBSERVATION_SOURCE', 'This integration does not accept Observation API records')
    ...
    if (schema && terrainValue) throw new AgentServiceError(400, 'NORMALIZATION_MODE_CONFLICT', 'Send either a versioned source schema or a canonical Terrain projection, not both')
    if (!schema && !terrainValue && integration.catalogId !== 'observation-api') {
      throw new AgentServiceError(400, 'NORMALIZATION_MODE_REQUIRED', 'This source requires a supported schema or a canonical Terrain projection')
    }
    ...
    if (schema) {
      try { normalized = normalizeSourceRecord(integration.catalogId, schema, payload, sourceRecordId) }
        ...
        if (cause instanceof SourceNormalizationError) throw new AgentServiceError(400, 'SOURCE_NORMALIZATION_FAILED', cause.message)
        throw cause
      }
    }
    ...
    if (value.evidenceType !== undefined && normalized && cleanText(value.evidenceType, 'evidenceType', 128) !== evidenceType) {
      throw new AgentServiceError(400, 'EVIDENCE_TYPE_MISMATCH', `Schema ${schema} produces ${evidenceType}`)
    }
    if (entry.evidenceTypes.length && !entry.evidenceTypes.includes(evidenceType)) throw new AgentServiceError(400, 'EVIDENCE_TYPE_NOT_ALLOWED', `${evidenceType} is not declared by this connector`)
    const entitiesValue = terrainValue?.entities ?? []
    const relationshipsValue = terrainValue?.relationships ?? []
    if (!Array.isArray(entitiesValue) || entitiesValue.length > 5_000) throw new AgentServiceError(400, 'INVALID_INPUT', 'terrain.entities must be an array with at most 5,000 entries')
    if (!Array.isArray(relationshipsValue) || relationshipsValue.length > 10_000) throw new AgentServiceError(400, 'INVALID_INPUT', 'terrain.relationships must be an array with at most 10,000 entries')
    ...
    try { return this.terrain.ingest(integration, observation) }
      ...
      if (cause instanceof SourceRecordConflictError) throw new AgentServiceError(409, 'SOURCE_RECORD_CONFLICT', cause.message)
      if (cause instanceof Error && /references an unknown entity/.test(cause.message)) throw new AgentServiceError(400, 'UNKNOWN_TERRAIN_ENTITY', cause.message)
      throw cause
    }
  }
  ...
  listInvestigations(principal: PortalPrincipal): AgentInvestigation[] {
    this.requirePortalAccess(principal)
    if (!this.actions) return []
    return this.actions.listInvestigations()
  }

  getInvestigation(principal: PortalPrincipal, id: string): AgentInvestigation {
    this.requirePortalAccess(principal)
    if (!this.actions) throw new AgentServiceError(404, 'NOT_FOUND', 'Action store not initialized')
  ...
  listProposals(principal: PortalPrincipal, investigationId?: string): AgentActionProposal[] {
    this.requirePortalAccess(principal)
    if (!this.actions) return []
  ...
  approveProposal(principal: PortalPrincipal, proposalId: string): AgentActionProposal {
    ...
    try {
      ...
      throw new AgentServiceError(409, 'PROPOSAL_STATE_ERROR', cause instanceof Error ? cause.message : 'Cannot approve proposal')
    }
  }

  denyProposal(principal: PortalPrincipal, proposalId: string, reason?: string): AgentActionProposal {
    requireRole(principal, 'Papyrus.Action.Approve')
    if (!this.actions) throw new AgentServiceError(503, 'ACTION_STORE_UNAVAILABLE', 'Action store is not initialized')
    const proposal = this.actions.getProposal(proposalId)
    if (!proposal) throw new AgentServiceError(404, 'PROPOSAL_NOT_FOUND', 'Action proposal not found')
    try {
      ...
      throw new AgentServiceError(409, 'PROPOSAL_STATE_ERROR', cause instanceof Error ? cause.message : 'Cannot deny proposal')
    }
  }
  ...
  private requireExecutable(executorIntegrationId: string): IntegrationConfiguration {
    ...
    if (!executor) throw new AgentServiceError(404, 'INTEGRATION_NOT_FOUND', 'Executor integration not found')
    ...
    if (!isApprovedEmailExecutor && executor.integrationClass !== 'action_executor' && executor.authority !== 'controlled_actions') {
      throw new AgentServiceError(400, 'NOT_AN_EXECUTOR', 'The specified integration is not an action executor')
    }
    if (executor.state !== 'active') throw new AgentServiceError(409, 'EXECUTOR_NOT_ACTIVE', 'Executor integration must be active')
  ...
  requirePortalAccess(principal: PortalPrincipal): void {
    if (principal.roles.length === 0) throw new AgentServiceError(403, 'ENTRA_APP_ROLE_REQUIRED', 'Papyrus access must be assigned through an Entra application role')
  }

  private integration(id: string): IntegrationConfiguration {
    const integration = this.db.getIntegration(id)
    if (!integration) throw new AgentServiceError(404, 'INTEGRATION_NOT_FOUND', 'Integration not found')
    return integration
  }
}
