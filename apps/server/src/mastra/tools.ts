...
import { z } from 'zod'
import { createTextPdf, normalizePdfFilename } from '../pdf.js'
...
export function buildStaticAgentTools() {
  ...
  const createPdf = createTool({
    id: 'papyrus_create_pdf',
    description: 'Create a PDF directly in the current Papyrus session workspace. Use this for PDF generation or export requests instead of shell commands, Python, Node scripts, or system PDF utilities. Broad requests may use the defaults immediately.',
    inputSchema: z.object({
      filename: z.string().max(180).default('document.pdf').describe('Output filename. .pdf is added automatically when omitted.'),
      title: z.string().max(200).default('Papyrus Document').describe('Document title.'),
      body: z.string().max(100_000).default('Generated by Papyrus.').describe('Plain-text document body. Use reasonable default content when the user did not specify content.'),
    }),
    execute: async ({ filename, title, body }, context) => {
      const writeArtifact = context?.requestContext?.get('writeArtifact') as ((filename: string, data: Buffer) => Promise<string>) | undefined
      if (!writeArtifact) throw new Error('Session artifact writing is unavailable')
      const outputName = normalizePdfFilename(filename)
      const pdf = createTextPdf({ title, body })
      const path = await writeArtifact(outputName, pdf)
      return { type: 'text', text: `Created ${path} (${pdf.byteLength} bytes, application/pdf)` }
    },
  })
