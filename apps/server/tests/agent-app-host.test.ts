...
import { appOrigin,handleAppPlane } from '../src/agent/apps/http.js'
...
const portal='https://portal.example.test', dedicatedAppOrigin='https://apps.example.test'
...
describe('hosted app authentication boundary',()=>{
 it('defaults app content to the portal transport origin while keeping an opaque sandbox',async()=>{
  expect(appOrigin(portal)).toBe(portal)
  ...
  try{
   ...
   const frame=JSON.parse(shell.body.match(/const frame=(\{.*?\}),appId=/)?.[1]??'{}') as {url:string;ticket:string;nonce:string}
   expect(frame.url).toBe(`${portal}/app-content`);expect(frame.ticket).toBeTruthy()
   const content=response(),payload=new URLSearchParams({ticket:frame.ticket,nonce:frame.nonce}).toString()
   await handleAppPlane(request('POST',portal,'/app-content',payload),content.res,new URL(`${portal}/app-content`),service,auth,runtime)
   expect(content.status).toBe(200);expect(content.body).toContain('Visible');expect(content.headers['content-security-policy']).toContain('sandbox allow-scripts');expect(content.headers['content-security-policy']).toContain("connect-src 'none'")
   await expect(handleAppPlane(request('POST',portal,'/app-content',payload),response().res,new URL(`${portal}/app-content`),service,auth,runtime)).rejects.toThrow(/expired/)
  }finally{db.close()}
 })
 it('still supports a dedicated app content origin when configured',async()=>{
  process.env.PAPYRUS_APP_ORIGIN=dedicatedAppOrigin
  expect(appOrigin(portal)).toBe(dedicatedAppOrigin)
  const db=new AgentDatabase(':memory:'),apps=new AppStore(db),policies=new PolicyStore(db)
  try{
   const app=apps.create('Intake','owner','thread'),release=apps.candidate(app.id,'source','<!doctype html><html><body><main>Visible</main></body></html>')
   policies.approve(apps.requestPublish(app.id,release.id,'owner'),actor,(k,r,v,b)=>apps.apply(k,r,v,b,'owner'))
   ...
   expect(shell.status).toBe(200)
   const frame=JSON.parse(shell.body.match(/const frame=(\{.*?\}),appId=/)?.[1]??'{}') as {url:string;ticket:string;nonce:string}
   expect(frame.url).toBe(`${dedicatedAppOrigin}/app-content`)
   const content=response(),payload=new URLSearchParams({ticket:frame.ticket,nonce:frame.nonce}).toString()
   await handleAppPlane(request('POST',portal,'/app-content',payload),content.res,new URL(`${dedicatedAppOrigin}/app-content`),service,auth,runtime)
   expect(content.status).toBe(200);expect(content.body).toContain('Visible')
   const direct=response();await handleAppPlane(request('GET',dedicatedAppOrigin,'/app-content'),direct.res,new URL(`${dedicatedAppOrigin}/app-content`),service,auth,runtime)
