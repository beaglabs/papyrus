...
describe('audited invitation onboarding', () => {
  it('requires and atomically accepts an identity-bound invitation after bootstrap', () => {
    ...
    try {
      ...
      const invitation = ctx.service.createInvitation(activeOwner, {
        identityKind: 'email', identityValue: 'member@example.test', displayName: 'Member', role: 'User',
      })
      expect(invitation).toMatchObject({ status: 'pending', identityKind: 'email', identityValue: 'member@example.test', role: 'User', authMethod: 'oidc' })
      ...
      }, [{ kind: 'email', value: 'wrong@example.test' }])).toThrow('INVITATION_REQUIRED')
      ...
      }, [{ kind: 'email', value: 'member@example.test' }])
  ...
  it('matches a government pending identity by validated stable CAC selector without email', () => {
    const ctx = testContext()
    try {
      ctx.config.profile = 'government-il4'
      const owner = ctx.db.upsertUser({ externalId: 'x509:edipi:1000000001', displayName: 'Owner', authMethod: 'mtls' })
      ctx.db.setRole(owner.id, 'Owner')
      ctx.db.setSetting('bootstrapComplete', 'true')
      const invitation = ctx.service.createInvitation(ctx.db.getPrincipal(owner.id)!, {
        identityKind: 'edipi', identityValue: '1000000002', displayName: 'CAC User', role: 'User',
      })
      const accepted = ctx.db.resolveAuthenticatedUser({
        externalId: 'x509:edipi:1000000002', displayName: 'CAC User', authMethod: 'mtls',
      }, [
        { kind: 'edipi', value: '1000000002' },
        { kind: 'issuer_subject', value: 'aaaaaaaaaaaaaaaaaaaaaaaa:cn=cac user' },
      ])
      expect(accepted.principal).toMatchObject({ displayName: 'CAC User', roles: ['User'], authMethod: 'mtls' })
      expect(accepted.invitation).toMatchObject({ id: invitation.id, status: 'accepted', identityKind: 'edipi' })
    } finally {
      ctx.dispose()
    }
  })
  ...
  it('prevents Admins from inviting privileged roles and audits the denial', () => {
    ...
    try {
      ...
      expect(() => ctx.service.createInvitation(activeAdmin, {
        identityKind: 'email', identityValue: 'owner@example.test', displayName: 'Owner', role: 'Owner',
