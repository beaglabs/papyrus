...
export function App() {
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="runtime-status"><span className={data.health.goose.available ? 'dot good' : 'dot'} />Goose {data.health.goose.available ? 'ready' : 'offline'}</div>
    </aside>
    <main>
      <header>
...
function Overview({ data }: { data: DashboardData }) {
  return <>
    ...
    <section className="grid-two">
      ...
      <article className="panel"><div className="panel-head"><h2>Control plane</h2><span className="status-good">ENFORCED</span></div><dl className="facts"><div><dt>Policy engine</dt><dd>Cedar {data.health.cedar}</dd></div><div><dt>Runtime</dt><dd>Goose only</dd></div><div><dt>Server mode</dt><dd>{data.health.mode}</dd></div><div><dt>Identity</dt><dd>{data.me.authMethod}</dd></div></dl></article>
    </section>
  </>
}
...
function Admin({ data, onChanged }: { data: DashboardData; onChanged: () => Promise<void> }) {
  ...
  const submitRuntime = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); void run(() => api('/api/runtimes', { method: 'POST', body: JSON.stringify({ name: values.get('name'), model: { provider: values.get('provider'), baseUrl: values.get('baseUrl'), model: values.get('model'), secretRef: values.get('secretRef') } }) }), form) }
  ...
  return <>
    ...
    <section className="admin-grid">
      ...
      <article className="panel accent"><h2>Resource assignment</h2><form className="stack" onSubmit={submitAssignment}><label>User<select name="user">{data.users.map((user) => <option value={user.id} key={user.id}>{user.displayName}</option>)}</select></label><label>Workspace or runtime<select name="resource"><optgroup label="Workspaces">{data.workspaces.map((item) => <option value={`workspace:${item.id}`} key={item.id}>{item.name}</option>)}</optgroup><optgroup label="Goose runtimes">{data.runtimes.map((item) => <option value={`runtime:${item.id}`} key={item.id}>{item.name}</option>)}</optgroup></select></label><button className="primary">Assign resource →</button></form></article>
      <article className="panel"><div className="panel-head"><h2>Goose runtimes</h2><span>{data.runtimes.length}</span></div>{data.runtimes.length ? <div className="rows">{data.runtimes.map((runtime) => <div className="row" key={runtime.id}><div><strong>{runtime.name}</strong><small>{runtime.model.provider} · {runtime.model.model}</small></div><span className="pill ready">GOOSE</span></div>)}</div> : <Empty>No runtime configured.</Empty>}<form className="stack mini-form" onSubmit={submitRuntime}><label>Name<input name="name" required placeholder="IL6 local inference" /></label><label>Provider<select name="provider"><option value="openai-compatible">OpenAI-compatible</option><option value="azure-openai">Azure OpenAI</option><option value="anthropic-compatible">Anthropic-compatible</option></select></label><label>Base URL<input name="baseUrl" required placeholder="https://model.internal/v1" /></label><label>Model<input name="model" required placeholder="approved-model-id" /></label><label>Secret reference<input name="secretRef" required placeholder="model_primary" /></label><button className="primary">Add Goose runtime →</button></form></article>
