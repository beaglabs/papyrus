  ...
  return <div className="environment-console">
    ...
    <section className="environment-detail">
      {creating ? <form className="create-session environment-create-panel" onSubmit={(event) => {
        ...
        <Card className="panel environment-hero"><p className="eyebrow">CEDAR AUTHORIZATION BOUNDARY</p><h2>{selected.name}</h2><p>{selected.description || 'No purpose or handling description has been provided.'}</p></Card>
        <div className="environment-controls">
          <Card className="panel admin-panel"><div className="panel-head"><h2>Members</h2><span>{selected.assignedUserIds.length}</span></div>
            {privileged && data ? <div className="resource-list">
              {selected.assignedUserIds.length ? selected.assignedUserIds.flatMap((id) => {
                const user = data.users.find((item) => item.id === id)
                return user ? [<Card className="resource-list-card" key={id}><div><strong>{user.displayName}</strong><span>{authenticationLabel(user.authMethod)} · {user.roles.join(' · ') || 'No role'}</span></div><span className="pill completed">member</span></Card>] : []
          ...
          <Card className="panel admin-panel"><div className="panel-head"><h2>MCP access</h2><span>{grantedServerIds.size}</span></div>
            {privileged && data ? <><p className="admin-note">Enabled sources expose tools to this environment. Cedar policy and approval still govern every call.</p><div className="resource-list">
              {data.toolGrants.filter((grant) => grant.environmentId === selected.id).length ? data.toolGrants.filter((grant) => grant.environmentId === selected.id).map((grant) => {
                ...
                return <Card className="resource-list-card" key={grant.id}><div><strong>{server?.name ?? 'Registered server'}</strong><span>{server?.endpoint ?? 'Governed MCP tool source'}</span></div><Button className="danger" disabled={busy} onClick={() => void act(() => revokeToolGrant(grant.id))}>Disable</Button></Card>
