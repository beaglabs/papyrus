...
export function ModelGatewayCard({ request, onChanged }: { request: { fields: Array<{ name: string; label: string; kind: string; required: boolean; placeholder?: string; help?: string; options?: Array<{ label: string; value: string }> }>; note: string }; onChanged: () => Promise<void> }) {
  ...
  if (state === 'connected') return <Card className="agent-setup-card connected"><span className="tool-icon">✓</span><div><strong>Model gateway configured</strong><p>The daemon tested the endpoint and recorded only non-secret profile metadata.</p></div></Card>
  return <Card className="agent-setup-card model-gateway-card"><div className="agent-setup-head"><div><p className="eyebrow">SECURE MODEL SETUP</p><h3>Configure model gateway</h3></div><Badge>DAEMON-OWNED</Badge></div><p>The daemon owns the connection. If authentication is required, enter only the environment variable name; never paste the API key.</p><form onSubmit={(event) => void submit(event)}><div className="agent-form-grid">{request.fields.map((field) => {
    if (field.name === 'credentialRef' && authMode !== 'api_key') return null
    return <label className="nb-label" key={field.name}>{field.label}{field.kind === 'select' ? <select className="nb-select" name={field.name} required={field.required} value={field.name === 'authScheme' ? authMode : undefined} onChange={field.name === 'authScheme' ? (event) => setAuthMode(event.target.value) : undefined}>{field.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <input className="nb-input" name={field.name} type={field.kind === 'credential_reference' ? 'text' : field.kind} required={field.required} placeholder={field.placeholder} autoComplete="off" />}{field.help && <small>{field.help}</small>}</label>
  })}</div>{error && <Alert className="error">{error}</Alert>}<div className="agent-setup-foot"><small>{request.note}</small><Button className="primary" disabled={state === 'saving'}>{state === 'saving' ? 'Testing…' : 'Save and test gateway'}</Button></div></form></Card>
}
