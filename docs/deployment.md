...
The branch implements deployment configuration, Entra validation, connector governance, portal routes, licensing, and audit persistence. Live Teams commands, Exchange polling, vendor telemetry drivers, and the Starlings process adapter are separate runtime slices and are not represented as operational merely because a connector has been saved.
