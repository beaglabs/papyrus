...
- **Kernel-backed isolation, no fallback.** The active workspace boundary is `landstrip`, which applies Landlock on Linux and Seatbelt on macOS, plus seccomp for syscalls. Papyrus spawns it as a worker, resolves the effective policy, and validates it before running anything, so a policy it cannot prove is a startup refusal rather than an unisolated run. Linux is the supported deployment target, macOS supports local development, and Windows has execution switched off. The legacy `PAPYRUS_SANDBOX_RUNTIME=bwrap|seatbelt` policy module is retained for configuration compatibility and is covered by tests only — no production path applies Bubblewrap.
- **No unisolated fallback.** If the platform is unsupported, or `landstrip` cannot apply its policy, execution is disabled and the daemon logs the specific reason. It does not proceed without isolation.
...
- Sandboxed execution requires Linux with Landlock, or macOS with Seatbelt for local development. On other hosts the rest of the daemon is fully functional, but agent code execution is off.
