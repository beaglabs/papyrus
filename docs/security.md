...
- **Kernel-backed isolation, no fallback.** The active workspace boundary is `nono-ts`, which applies Landlock on Linux and Seatbelt on macOS. Linux is the supported deployment target, macOS supports local development, and Windows has execution switched off. The legacy `PAPYRUS_SANDBOX_RUNTIME=bwrap|seatbelt` policy module is retained for configuration compatibility and is covered by tests only — no production path applies Bubblewrap.
- **No unisolated fallback.** If the platform is unsupported, or `nono-ts` cannot apply its capability set, execution is disabled and the daemon logs the specific reason. It does not proceed without isolation.
