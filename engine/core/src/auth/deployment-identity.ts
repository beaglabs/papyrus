...
export function loadOrGenerateDeploymentIdentity(dir = DEFAULT_CONFIG_DIR): DeploymentIdentity {
  ...
  if (existsSync(path)) {
    ...
    if (deploymentFingerprint(identity.publicKeyPem) !== identity.deploymentId) throw new Error('Deployment identity fingerprint mismatch')
    return identity
  }
  ...
  const identity: DeploymentIdentity = { publicKeyPem: publicKey, privateKeyPem: privateKey, deploymentId: deploymentFingerprint(publicKey), createdAt: new Date().toISOString() }
  mkdirSync(dir, { recursive: true, mode: 0o700 })
  writeFileSync(path, JSON.stringify(identity, null, 2), { encoding: 'utf8', mode: 0o600, flag: 'wx' })
  return identity
}
