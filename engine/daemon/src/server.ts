...
import {
  generateAuthorizationEvidence,
  getDeploymentPosture,
  updateDeploymentPosture,
} from './deployment-authorization.js'
import {
...
import {
  getIntakeSecuritySettings,
  overrideSecurityResult,
  scanIntakeItem,
...
async function handleAPI(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  if (url.pathname === '/api/intake' && method === 'GET') {
    ...
    json(res, 200, { items: listIntake(membership.org.id) })
    return true
  }
  if (url.pathname === '/api/cape/connections' && method === 'GET') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    json(res, 200, {
      connections: listCapeConnections(membership.org.id),
      canManage: membership.role === 'admin',
    })
    return true
  }
  if (url.pathname === '/api/admin/model-evaluations' && method === 'GET') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    json(res, 200, {
      runs: listEvaluationRuns(membership.org.id),
      canManage: membership.role === 'admin',
    })
    return true
  }
  if (url.pathname === '/api/admin/model-evaluations' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    ...
    try {
      json(
        res,
        201,
        runPinnedEvaluation(membership.org.id, authCtx.memberKey, body as Record<string, unknown>),
      )
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Evaluation failed' })
    }
    return true
  }
  if (url.pathname === '/api/records' && method === 'GET') {
    ...
    json(res, 200, {
      schedules: ensureDefaultSchedules(membership.org.id, authCtx.memberKey),
      holds: listLegalHolds(membership.org.id),
      canManage: membership.role === 'admin',
    })
    return true
  }
  if (url.pathname === '/api/admin/deployment-posture' && method === 'GET') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    json(res, 200, {
      posture: getDeploymentPosture(membership.org.id, authCtx.memberKey),
      canManage: membership.role === 'admin',
    })
    return true
  }
  if (url.pathname === '/api/admin/deployment-posture' && method === 'PUT') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
    ...
    try {
      json(res, 200, {
        posture: updateDeploymentPosture(membership.org.id, authCtx.memberKey, body),
      })
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Posture update failed' })
    }
    return true
  }
  if (url.pathname === '/api/admin/authorization-evidence' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
    ...
    json(res, 201, generateAuthorizationEvidence(membership.org.id, authCtx.memberKey))
    return true
  }
  if (url.pathname === '/api/records/assign' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    try {
      ...
      json(res, 400, {
        error: error instanceof Error ? error.message : 'Schedule assignment failed',
      })
    }
    return true
  }
  if (url.pathname === '/api/records/holds' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
      json(res, 403, { error: 'Records administrator required' })
      return true
    }
    const body = await parseBody(req)
    try {
      ...
      json(res, 400, { error: error instanceof Error ? error.message : 'Hold creation failed' })
    }
    return true
  }
  if (url.pathname === '/api/cape/connections' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
    ...
    const body = await parseBody(req)
    try {
      json(res, 201, saveCapeConnection(membership.org.id, authCtx.memberKey, body))
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Connection invalid' })
    }
    return true
  }
  if (url.pathname === '/api/cape/connections/test' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
    ...
    const body = await parseBody(req)
    try {
      json(res, 200, testCapeConnection(membership.org.id, String(body.id ?? '')))
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Connection test failed' })
    }
    return true
  }
  if (url.pathname === '/api/cape/connections/queue' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    try {
      json(
        ...
        enqueueConnectionAction(
          ...
          (typeof body.payload === 'object' && body.payload ? body.payload : {}) as Record<
            string,
            unknown
          >,
        ),
      ...
      json(res, 400, { error: error instanceof Error ? error.message : 'Queue failed' })
    }
    return true
  }
    ...
    try {
      json(
        ...
        stageIntake({
          ...
          submittedBy: authCtx.memberKey,
        }),
      )
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Intake failed' })
    }
    return true
  }
    ...
    try {
      json(
        ...
        decideIntake(String(body.id), membership.org.id, {
          ...
          tags: Array.isArray(body.tags) ? body.tags.map(String) : [],
          rationale: typeof body.rationale === 'string' ? body.rationale : undefined,
          reviewedBy: authCtx.memberKey,
        }),
      )
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Decision failed' })
    }
    return true
  }
  if (url.pathname === '/api/intake/process' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    const body = await parseBody(req)
    try {
      json(res, 200, processDocumentJob(String(body.id ?? ''), membership.org.id))
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Processing failed' })
    }
    return true
  }
  if (url.pathname === '/api/admin/document-processing' && method === 'GET') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    json(res, 200, {
      settings: getDocumentProcessingSettings(membership.org.id),
      canManage: membership.role === 'admin',
    })
    return true
  }
  if (url.pathname === '/api/intake/security/scan' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    const body = await parseBody(req)
    try {
      json(res, 200, scanIntakeItem(membership.org.id, String(body.id ?? '')))
    } catch (error) {
      json(res, 400, { error: error instanceof Error ? error.message : 'Scan failed' })
    }
    return true
  }
  if (url.pathname === '/api/intake/security/override' && method === 'POST') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership || membership.role !== 'admin') {
      json(res, 403, { error: 'Security administrator required' })
      return true
    }
    const body = await parseBody(req)
    try {
      ...
      json(res, 400, { error: error instanceof Error ? error.message : 'Override failed' })
    }
    return true
  }
  if (url.pathname === '/api/admin/intake-security' && method === 'GET') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
  ...
  if (url.pathname === '/api/admin/intake-security' && method === 'PUT') {
    ...
    const body = await parseBody(req)
    json(res, 200, {
      settings: updateIntakeSecuritySettings(membership.org.id, body, authCtx.memberKey),
    })
    return true
  }
  if (url.pathname === '/api/admin/document-processing' && method === 'PUT') {
    const authCtx = requireAuth(req, res)
    if (!authCtx) return true
    const membership = getOrgForMember(authCtx.memberKey)
    if (!membership) {
    ...
    try {
      json(res, 200, {
        settings: updateDocumentProcessingSettings(
          membership.org.id,
          {
            maxFileSizeBytes:
              typeof body.maxFileSizeBytes === 'number' ? body.maxFileSizeBytes : undefined,
            ocrEnabled: typeof body.ocrEnabled === 'boolean' ? body.ocrEnabled : undefined,
            ocrLanguages: Array.isArray(body.ocrLanguages)
              ? body.ocrLanguages.map(String)
              : undefined,
            nativeTextMinimum:
              typeof body.nativeTextMinimum === 'number' ? body.nativeTextMinimum : undefined,
            jobTimeoutSeconds:
              typeof body.jobTimeoutSeconds === 'number' ? body.jobTimeoutSeconds : undefined,
            retainIntermediates:
              typeof body.retainIntermediates === 'boolean' ? body.retainIntermediates : undefined,
          },
          authCtx.memberKey,
        ),
      })
      ...
      json(res, 400, {
        error: error instanceof Error ? error.message : 'Invalid processing settings',
      })
    }
    return true
  }
